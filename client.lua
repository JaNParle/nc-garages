...
local occupiedParkingSpots = {}
local activeStoreTargets = {}
local activeStoreTargetEntity = nil

local function RemoveStoreTarget(entity)
    if entity and DoesEntityExist(entity) and activeStoreTargets[entity] then
        exports['qb-target']:RemoveTargetEntity(entity, "park_vehicle_target")
        activeStoreTargets[entity] = nil
        if activeStoreTargetEntity == entity then
            activeStoreTargetEntity = nil
        end
    end
end

local function RemoveAllStoreTargets()
    for entity, _ in pairs(activeStoreTargets) do
...
local function AddStoreTarget(entity, garageId, garageType, isJobVehicle, jobName)
    if not entity or entity == 0 or not DoesEntityExist(entity) then return end
    if activeStoreTargets[entity] then return end

    exports['qb-target']:AddTargetEntity(entity, {
        options = {
            {
                icon = "fas fa-warehouse",
                label = "Parchează Vehicul",
                action = function()
                    if isJobVehicle and jobName then
                        ParkJobVehicle(entity, jobName)
                    else
                        TriggerEvent('nc-garages:client:StoreVehicle', {
                            garageId = garageId,
                            garageType = garageType
                        })
                    end
                end,
                canInteract = function(targetEntity, distance, data)
                    if not DoesEntityExist(targetEntity) then return false end

                    local ped = PlayerPedId()
                    if IsPedInAnyVehicle(ped, false) then return false end

                    local pedInDriverSeat = GetPedInVehicleSeat(targetEntity, -1)
                    if pedInDriverSeat ~= 0 then return false end

                    local speed = GetEntitySpeed(targetEntity)
                    if speed >= 0.1 then return false end

                    local targetPlate = QBCore.Functions.GetPlate(targetEntity)
                    ...
                    return vehicleOwnershipCache[targetPlate] == true
                end
            }
        },
        distance = 2.5
    ...
    activeStoreTargets[entity] = true
    activeStoreTargetEntity = entity
end
...
function ParkJobVehicle(vehicle, jobName)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('nc-garages:server:TrackJobVehicle', plate, jobName, props)
...
RegisterNetEvent('nc-garages:client:DeleteGarage')
AddEventHandler('nc-garages:client:DeleteGarage', function(garageId)
    -- Simply forward to server
    TriggerServerEvent('nc-garages:server:DeleteSharedGarage', garageId)
end)
...
RegisterNUICallback('confirmRemoveVehicle', function(data, cb)
    ...
    local removeMenu = {
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = "nc-garages:server:RemoveVehicleFromSharedGarage",
                args = {
                    plate = plate
                }
            }
...
RegisterNetEvent('nc-garages:client:ConfirmDeleteGarage', function(data)
    TriggerServerEvent('nc-garages:server:DeleteSharedGarage', data.garageId)
...
RegisterNetEvent('nc-garages:client:CancelDeleteGarage')
AddEventHandler('nc-garages:client:CancelDeleteGarage', function()
    -- Do nothing, just catch the event
end)


RegisterNetEvent('nc-garages:client:ConfirmRemoveVehicle', function(data)
    TriggerServerEvent('nc-garages:server:RemoveVehicleFromSharedGarage', data.plate)
...
RegisterNetEvent('nc-garages:client:CancelRemoveVehicle', function(data)
...
RegisterNUICallback('confirmDeleteGarage', function(data, cb)
    ...
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "nc-garages:client:ConfirmDeleteSharedGarage",
                args = {
                    garageId = garageId
                }
            }
        },
        {
            ...
            params = {
                event = "nc-garages:client:CancelDeleteGarage"
            }
        }
    })
...
RegisterNetEvent('nc-garages:client:ConfirmDeleteSharedGarage')
AddEventHandler('nc-garages:client:ConfirmDeleteSharedGarage', function(data)
    local garageId = data.garageId
...
RegisterNUICallback('refreshVehicles', function(data, cb)
    ...
    if garageType == "public" then
        QBCore.Functions.TriggerCallback('nc-garages:server:GetPersonalVehicles', function(vehicles)
            if vehicles then
                SendNUIMessage({
    ...
    elseif garageType == "gang" then
        ...
        QBCore.Functions.TriggerCallback('nc-garages:server:GetGangVehicles', function(vehicles)
            if vehicles then
                SendNUIMessage({
                    action = "refreshVehicles",
                    vehicles = FormatVehiclesForNUI(vehicles)
    ...
    elseif garageType == "shared" then
        QBCore.Functions.TriggerCallback('nc-garages:server:GetSharedGarageVehicles', function(vehicles)
            if vehicles then
                SendNUIMessage({
                    action = "refreshVehicles",
                    vehicles = FormatVehiclesForNUI(vehicles)
    ...
    elseif garageType == "impound" then
        QBCore.Functions.TriggerCallback('nc-garages:server:GetImpoundedVehicles', function(vehicles)
            if vehicles then
                SendNUIMessage({
                    action = "refreshVehicles",
                    vehicles = FormatVehiclesForNUI(vehicles)
    ...
    while true do
        ...
        for plate, _ in pairs(trackedVehicles) do
            if not currentVehicles[plate] then
                ...
                TriggerServerEvent('nc-garages:server:HandleDeletedVehicle', plate)
...
RegisterNUICallback('checkVehicleState', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:CheckVehicleStatus', function(isStored)
        if isStored then
...
RegisterCommand('debuggarage', function(source, args)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:GetJobGarageVehicles', function(vehicles)        
...
function IsVehicleOwned(vehicle)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:CheckIfVehicleOwned', function(owned)
    ...
    while true do
                ...
                if pedInDriverSeat == 0 and isStationary then
                        ...
                        if jobGarage and jobGarage.job == jobName then
                            ...
                            if isJobVehicle then
                                if distToVehicle < 10.0 then
                                    ...
                                    AddStoreTarget(vehicle, garageId, garageType, true, jobName)
                                    addedTarget = true
                                else
                    ...
                    if isOwned then
                        if distToVehicle < 10.0 then
                            ...
                            AddStoreTarget(vehicle, garageId, garageType, false, nil)
                            addedTarget = true
                        else
                            if isVehicleFaded and fadedVehicle == vehicle then
                                SetEntityAlpha(vehicle, 255, false)
                    ...
                    if not addedTarget and activeStoreTargetEntity and activeStoreTargetEntity ~= vehicle then
                        RemoveStoreTarget(activeStoreTargetEntity)
                    end
        ...
        elseif isInVehicle then
            ...
            if currentVehicle > 0 and DoesEntityExist(currentVehicle) then
                ...
                if plate then
                    QBCore.Functions.TriggerCallback('nc-garages:server:CheckJobAccess', function(hasAccess)
...
RegisterNetEvent('nc-garages:client:FreeJobParkingSpot', function(jobName, spotIndex)
            ...
            if data.garageType == "public" then
                exports['qb-target']:AddTargetEntity(data.ped, {
                    options = {
                        {
                            type = "client",
                            event = "nc-garages:client:OpenGarage",
                            icon = "fas fa-car",
                            label = "Open Garage",
            ...
            elseif data.garageType == "impound" then
                exports['qb-target']:AddTargetEntity(data.ped, {
                    options = {
                        {
                            type = "client",
                            event = "nc-garages:client:OpenImpoundLot",
                            icon = "fas fa-car",
...
while true do
    ...
    for k, v in pairs(Config.Garages) do
        ...
        if dist <= 3.0 then 
            ...
            if IsControlJustReleased(0, 38) then
                TriggerEvent("nc-garages:client:OpenGarage", {garageId = k, garageType = "public"})
            end
        end
    end
    ...
    for k, v in pairs(Config.JobGarages) do
        if PlayerData.job and PlayerData.job.name == v.job then
            ...
            if dist <= 3.0 then 
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("nc-garages:client:OpenGarage", {garageId = k, garageType = "job"})
                end
            end
        end
    end
    ...
    for k, v in pairs(Config.GangGarages) do
        if PlayerData.gang and PlayerData.gang.name == v.gang then
            ...
            if dist <= 3.0 then 
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("nc-garages:client:OpenGarage", {garageId = k, garageType = "gang"})
                end
            end
        end
    end
    ...
    for k, v in pairs(Config.ImpoundLots) do
        ...
        if dist <= 3.0 then 
            ...
            if IsControlJustReleased(0, 38) then
                TriggerEvent("nc-garages:client:OpenImpoundLot", {impoundId = k})
            end
        end
    end
...
function OpenGarageUI(vehicles, garageInfo, garageType)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:GetAllGarages', function(allGarages)
...
RegisterNetEvent('nc-garages:client:OpenGarage', function(data)
...
RegisterNetEvent('nc-garages:client:CloseGarage')
AddEventHandler('nc-garages:client:CloseGarage', function()
    SetNuiFocus(false, false)
end)
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:GetVehicleByPlate', function(vehData, isOut)
        if isOut then
        ...
        if garageType == "shared" then
            QBCore.Functions.TriggerCallback('nc-garages:server:CheckSharedAccess', function(hasAccess)
                if hasAccess then
                    TriggerServerEvent('nc-garages:server:TakeOutSharedVehicle', plate, garageId)
                else
        ...
        QBCore.Functions.SpawnVehicle(model, function(veh)
            ...
            if garageType == "public" or garageType == "gang" then
                QBCore.Functions.TriggerCallback('nc-garages:server:GetVehicleProperties', function(properties)
                    if properties then
                        ...
                        TriggerServerEvent('nc-garages:server:UpdateVehicleState', plate, 0)
                        
                        if garageType == "gang" and data.storedInGang then
                            TriggerServerEvent('nc-garages:server:UpdateGangVehicleState', plate, 0)
                        end
...
RegisterNetEvent('nc-garages:client:TakeOutSharedVehicle', function(plate, vehicleData)
    local garageId = currentGarage.id
    local garageType = currentGarage.type
...
function PlayVehicleTransferAnimation(plate, fromGarageId, toGarageId)
    ...
    if not garageInfo then 
        TriggerServerEvent('nc-garages:server:TransferVehicleToGarage', plate, toGarageId, Config.TransferCost or 500)
...
RegisterNUICallback('directTransferVehicle', function(data, cb)
    ...
    if Config.EnableTransferAnimation then
        ...
        TriggerServerEvent('nc-garages:server:TransferVehicleToGarage', plate, newGarageId, cost)
    end
    
    -- Give some time for the server to process before refreshing
    Citizen.SetTimeout(1000, function()
        TriggerEvent('nc-garages:client:RefreshVehicleList')
    end)
end)
...
RegisterNetEvent("nc-garages:client:PlayTransferAnimation", function(plate, newGarageId)
    local ped = PlayerPedId()
    local garageType = currentGarage.type
    ...
    Wait(3000)
    ClearPedTasks(ped)
    TriggerServerEvent('nc-garages:server:TransferVehicleToGarage', plate, newGarageId, Config.TransferCost or 500)
    transferAnimationActive = false
    Wait(1000)
    isTransferringVehicle = false
    currentTransferVehicle = nil
...
RegisterNetEvent('nc-garages:client:TransferComplete', function(newGarageId, plate)
...
RegisterNUICallback('updateVehicleName', function(data, cb)
    ...
    if plate and newName then
        TriggerServerEvent('nc-garages:server:UpdateVehicleName', plate, newName)
        cb({status = "success"})
    else
        cb({status = "error", message = "Invalid data"})
    end
...
RegisterNUICallback('toggleFavorite', function(data, cb)
    ...
    if plate then
        TriggerServerEvent('nc-garages:server:ToggleFavorite', plate, isFavorite)
        cb({status = "success"})
    else
...
RegisterNUICallback('storeInGang', function(data, cb)
    ...
    if plate and gangName then
        TriggerServerEvent('nc-garages:server:StoreVehicleInGang', plate, gangName)
        cb({status = "success"})
    else
        cb({status = "error", message = "Invalid data"})
    end
...
RegisterNUICallback('removeFromShared', function(data, cb)
    ...
    if plate then
        TriggerServerEvent('nc-garages:server:RemoveVehicleFromSharedGarage', plate)
        cb({status = "success"})
    else
        cb({status = "error", message = "Invalid plate"})
    end
...
function OpenSharedGarageSelectionUI(plate)
    QBCore.Functions.TriggerCallback('nc-garages:server:GetSharedGarages', function(garages)
        if #garages == 0 then
...
RegisterNUICallback('storeInSelectedSharedGarage', function(data, cb)
    ...
    TriggerServerEvent('nc-garages:server:TransferVehicleToSharedGarage', plate, garageId)
...
RegisterNetEvent('nc-garages:client:StoreVehicle', function(data)
    local ped = PlayerPedId()
    local garageId = nil
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:CheckOwnership', function(isOwner, isInGarage)
        if isOwner or (garageType == "gang" and isInGarage) then
            FadeOutVehicle(curVeh, function()
                TriggerServerEvent('nc-garages:server:StoreVehicle', plate, garageId, props, fuel, engineHealth, bodyHealth, garageType)
...
RegisterNetEvent('nc-garages:client:ManageSharedGarages')
AddEventHandler('nc-garages:client:ManageSharedGarages', function()
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:CheckSharedGaragesTables', function(tablesExist)
        if not tablesExist then
            TriggerServerEvent('nc-garages:server:CreateSharedGaragesTables')
...
RegisterNUICallback('manageSharedGarages', function(data, cb)
    TriggerEvent('nc-garages:client:ManageSharedGarages')
    cb({status = "success"})
end)
...
RegisterNUICallback('createSharedGarage', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:CreateSharedGarage', function(success, result)
        if success then
...
RegisterNUICallback('joinSharedGarage', function(data, cb)
    ...
    TriggerServerEvent('nc-garages:server:RequestJoinSharedGarage', accessCode)
    cb({status = "success"})
end)
...
RegisterNUICallback('openSharedGarage', function(data, cb)
    ...
    TriggerEvent('nc-garages:client:OpenGarage', {
        garageId = garageId,
        garageType = "shared",
...
RegisterNUICallback('manageSharedGarageMembers', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:GetSharedGarageMembers', function(members)
        if members then
            SendNUIMessage({
...
RegisterNUICallback('removeSharedGarageMember', function(data, cb)
    ...
    TriggerServerEvent('nc-garages:server:RemoveMemberFromSharedGarage', memberId, garageId)
    cb({status = "success"})
end)
...
RegisterNetEvent('nc-garages:client:ReceiveJoinRequest', function(data)
...
RegisterNUICallback('handleJoinRequest', function(data, cb)
    ...
    if approved then
        TriggerServerEvent('nc-garages:server:ApproveJoinRequest', requestData)
    else
        TriggerServerEvent('nc-garages:server:DenyJoinRequest', requestData)
    end
    
    cb({status = "success"})
end)

RegisterNetEvent('nc-garages:client:RefreshVehicleList', function()
...
RegisterNetEvent('nc-garages:client:VehicleTransferCompleted', function(successful, plate)
    if successful then
        if currentGarage and isMenuOpen then
            local garageId = currentGarage.id
            local garageType = currentGarage.type
...
function GetVehicleHoverInfo(vehicle)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:GetVehicleInfo', function(info)
        vehicleInfo = info
    end, plate)
...
RegisterNetEvent('nc-garages:client:OpenImpoundLot')
AddEventHandler('nc-garages:client:OpenImpoundLot', function(data)
...
RegisterCommand('impound', function(source, args)
    ...
    if dialog and dialog.reason then
        ...
        QBCore.Functions.Progressbar("impounding_vehicle", "Impounding Vehicle...", 10000, false, true, {
            ...
            TriggerServerEvent('nc-garages:server:ImpoundVehicleWithParams', plate, props, dialog.reason, impoundType, 
...
RegisterNUICallback('releaseImpoundedVehicle', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('nc-garages:server:CanPayImpoundFee', function(canPay)
        if canPay then
            local impoundInfo = Config.ImpoundLots[impoundId]
            ...
            QBCore.Functions.TriggerCallback('nc-garages:server:GetVehicleByPlate', function(vehData)
                if vehData then
                    QBCore.Functions.SpawnVehicle(vehData.vehicle, function(veh)
                        ...
                        QBCore.Functions.TriggerCallback('nc-garages:server:GetVehicleProperties', function(properties)
                            if properties then
                                ...
                                TriggerServerEvent('nc-garages:server:PayImpoundFee', plate, fee)
...
RegisterNetEvent('nc-garages:client:ImpoundVehicle')
AddEventHandler('nc-garages:client:ImpoundVehicle', function()
    if not PlayerData.job or not Config.ImpoundJobs[PlayerData.job.name] then
        QBCore.Functions.Notify("You are not authorized to impound vehicles", "error")
        return
    end
    ...
    if dialog and dialog.reason then
        ...
        QBCore.Functions.Progressbar("impounding_vehicle", "Impounding Vehicle...", 10000, false, true, {
            ...
            TriggerServerEvent('nc-garages:server:ImpoundVehicle', plate, props, dialog.reason, dialog.type, PlayerData.job.name, PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname)
