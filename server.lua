...
QBCore.Functions.CreateCallback('nc-garages:server:GetPersonalVehicles', function(source, cb, garageId)
...
QBCore.Functions.CreateCallback('nc-garages:server:GetVehiclesByGarage', function(source, cb, garageId)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb({}) end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetGangVehicles', function(source, cb, gang, garageId)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb({}) end
...
QBCore.Functions.CreateCallback('nc-garages:server:CheckOwnership', function(source, cb, plate, garageType)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateCallback('nc-garages:server:CheckSharedAccess', function(source, cb, plate, garageId)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb(false) end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetVehicleProperties', function(source, cb, plate)
...
QBCore.Functions.CreateCallback('nc-garages:server:GetAllGarages', function(source, cb)
    local garages = {}
...
RegisterNetEvent('nc-garages:server:TransferVehicleToGarage', function(plate, newGarageId, cost)
    local src = source
    ...
    MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE plate = ? AND citizenid = ?', {plate, citizenid}, function(result)
        ...
        MySQL.Async.execute('UPDATE player_vehicles SET garage = ? WHERE plate = ?', {newGarageId, plate}, function(rowsChanged)
            if rowsChanged > 0 then
                ...
                TriggerClientEvent('nc-garages:client:TransferComplete', src, newGarageId, plate)
            else
...
QBCore.Functions.CreateCallback('nc-garages:server:CheckJobVehicleAccess', function(source, cb, plate)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('nc-garages:server:TrackJobVehicle', function(plate, jobName, props, spotIndex)
    local src = source
...
RegisterNetEvent('nc-garages:server:FreeJobParkingSpot', function(jobName, spotIndex)
    if not jobName or not spotIndex then return end
    
    TriggerClientEvent('nc-garages:client:FreeJobParkingSpot', -1, jobName, spotIndex)
end)
-- Check if player has job access to a vehicle
QBCore.Functions.CreateCallback('nc-garages:server:CheckJobAccess', function(source, cb, plate)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb(false) end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetJobVehicleData', function(source, cb, plate)
    if trackedJobVehicles[plate] then
...
function ParkJobVehicle(vehicle, jobName)
    ...
    CreateThread(function()
        ...
        -- Store vehicle info
        TriggerServerEvent('nc-garages:server:TrackJobVehicle', plate, jobName, props, engineHealth, bodyHealth, fuelLevel, spotIndex)
...
RegisterNetEvent('nc-garages:server:StoreVehicle', function(plate, garageId, props, fuel, engineHealth, bodyHealth, garageType)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    local citizenid = Player.PlayerData.citizenid    
        ...
        MySQL.Async.fetchAll('SHOW COLUMNS FROM player_vehicles LIKE "state"', {}, function(stateColumn)
            ...
            MySQL.Async.execute(query, params, function(rowsChanged)
                if rowsChanged > 0 then
                    ...
                    TriggerClientEvent('nc-garages:client:RefreshVehicleList', src)
                else
...
RegisterNetEvent('nc-garages:server:UpdateGangVehicleState', function(plate, state)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:UpdateVehicleName', function(plate, newName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:ToggleFavorite', function(plate, isFavorite)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('qb-garage:server:ToggleFavorite', function(plate, isFavorite)
    TriggerEvent('nc-garages:server:ToggleFavorite', plate, isFavorite)
end)

RegisterNetEvent('nc-garages:server:StoreVehicleInGang', function(plate, gangName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('qb-garage:server:StoreVehicleInGang', function(plate, gangName)
    TriggerEvent('nc-garages:server:StoreVehicleInGang', plate, gangName)
end)
...
QBCore.Functions.CreateCallback('nc-garages:server:IsVehicleOut', function(source, cb, plate)
...
QBCore.Functions.CreateCallback('nc-garages:server:CreateSharedGarage', function(source, cb, garageName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-garages:server:RequestJoinSharedGarage', function(accessCode)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
            ...
            {garageData.id, citizenid}, function(memberResult)
                ...
                MySQL.Async.fetchAll('SELECT COUNT(*) as count FROM shared_garage_members WHERE garage_id = ?', 
                    {garageData.id}, function(countResult)
                        ...
                        TriggerClientEvent('nc-garages:client:ReceiveJoinRequest', ownerPlayer.PlayerData.source, {
...
RegisterNetEvent('nc-garages:server:ApproveJoinRequest', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:DenyJoinRequest', function(data)
    local src = source
    local requesterId = data.requesterId
...
QBCore.Functions.CreateCallback('nc-garages:server:GetSharedGarages', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then 
        return cb({}) 
...
QBCore.Functions.CreateCallback('nc-garages:server:GetSharedGarageVehicles', function(source, cb, garageId)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb({}) end
...
RegisterNetEvent('nc-garages:server:StoreVehicleInSharedGarage', function(plate, garageId, props, fuel, engineHealth, bodyHealth)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:UpdateVehicleState', function(plate, state)
...
RegisterNetEvent('nc-garages:server:RemoveVehicleFromSharedGarage', function(plate)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE plate = ? AND citizenid = ?', 
        {plate, citizenid}, function(result)
            if not result or #result == 0 then
                TriggerClientEvent('QBCore:Notify', src, "You don't own this vehicle", "error")
                TriggerClientEvent('nc-garages:client:VehicleTransferCompleted', src, false, plate)
                return
            end
            ...
            MySQL.Async.execute('UPDATE player_vehicles SET shared_garage_id = NULL WHERE plate = ?', 
                {plate}, function(rowsChanged)
                    if rowsChanged > 0 then
                        ...
                        TriggerClientEvent('nc-garages:client:VehicleTransferCompleted', src, true, plate)
...
RegisterNetEvent('nc-garages:server:TakeOutSharedVehicle', function(plate, garageId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
            ...
            MySQL.Async.execute('UPDATE player_vehicles SET state = 0, stored = 0 WHERE plate = ?', 
                {plate}, function(rowsChanged)
                    if rowsChanged > 0 then
                        ...
                        TriggerClientEvent('nc-garages:client:TakeOutSharedVehicle', src, plate, result[1])
                    else
...
QBCore.Functions.CreateCallback('nc-garages:server:CheckVehicleStatus', function(source, cb, plate)
    MySQL.Async.fetchAll('SELECT state FROM player_vehicles WHERE plate = ?', {plate}, function(result)
        if result and #result > 0 then
...
QBCore.Functions.CreateCallback('nc-garages:server:GetSharedGarageMembers', function(source, cb, garageId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return cb({}) end
...
RegisterNetEvent('nc-garages:server:HandleDeletedVehicle', function(plate)
...
RegisterNetEvent('nc-garages:server:RemoveMemberFromSharedGarage', function(memberId, garageId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:DeleteSharedGarage', function(garageId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('nc-garages:server:StoreInSelectedSharedGarage', function(source, cb, plate, garageId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nc-garages:server:TransferVehicleToSharedGarage', function(plate, garageId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('nc-garages:server:CheckIfVehicleOwned', function(source, cb, plate)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb(false) end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetVehicleInfo', function(source, cb, plate)
...
RegisterNetEvent('nc-garages:server:CreateSharedGaragesTables')
AddEventHandler('nc-garages:server:CreateSharedGaragesTables', function()
    local src = source
    MySQL.Async.execute([[
...
QBCore.Functions.CreateCallback('nc-garages:server:CheckSharedGaragesTables', function(source, cb)
    MySQL.Async.fetchAll("SHOW TABLES LIKE 'shared_garages'", {}, function(result)
        cb(result and #result > 0)
    end)
end)

QBCore.Functions.CreateCallback('nc-garages:server:CanPayImpoundFee', function(source, cb, fee)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb(false) end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetVehicleByPlate', function(source, cb, plate)
    if OutsideVehicles[plate] then
...
RegisterNetEvent('nc-garages:server:PayImpoundFee', function(plate, fee)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:ImpoundVehicle', function(plate, props, reason, impoundType, jobName, officerName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('nc-garages:server:ImpoundVehicleWithParams', function(plate, props, reason, impoundType, jobName, officerName, impoundFee)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetImpoundedVehicles', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return cb({}) end
...
QBCore.Functions.CreateCallback('nc-garages:server:GetJobGarageVehicles', function(source, cb, garageId)
